/**
 * Dual Index Search API Lambda Function - Direct VPC Connection
 * Supports both text search (cis-files) and image search (file-index-v2-knn)
 */

const { Client } = require('@opensearch-project/opensearch');

// OpenSearch client configuration - direct VPC connection
const createClient = () => {
  ...
  // Simple client configuration for VPC-to-VPC connection
  // No authentication needed as IAM roles handle this in VPC
  return new Client({
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (error.meta) {
      ...
      errorMessage += ` | Meta: ${JSON.stringify(error.meta)}`;
    }
