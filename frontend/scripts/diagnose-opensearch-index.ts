...
async function diagnoseOpenSearch() {
  ...
  try {
    ...
    try {
      ...
      const status =
        clusterHealth.body.status === 'green'
          ? 'OK'
          : clusterHealth.body.status === 'yellow'
            ? 'WARNING'
            : 'ERROR'
    ...
    try {
      ...
      hits.forEach((hit: any, index: number) => {
        ...
        console.log(
          `    image_embedding: ${source.image_embedding ? `存在 (${source.image_embedding.length}次元)` : '存在しない'}`
        )
    ...
    // 結果をJSON形式で保存
    const outputPath =
      '/Users/tatsuya/focus_project/cis_filesearch_app/frontend/scripts/opensearch-diagnosis-result.json'
