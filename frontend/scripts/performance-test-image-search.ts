...
class PerformanceTestRunner {
   ...
   * 画像埋め込みAPIをテスト
   */
  private async testEmbeddingAPI(imageFile: File): Promise<{ embedding: number[]; duration: number }> {
  ...
  private async testSingleRequest(imagePath: string): Promise<void> {
    ...
    try {
      ...
      console.log(`   ✅ Embedding generated: ${embeddingDuration.toFixed(2)}ms (${embedding.length} dimensions)`);
      ...
      console.log(`   ✅ Search completed: ${searchDuration.toFixed(2)}ms (${results.length} results)`);
      ...
      console.log(`   📊 Total time: ${totalDuration.toFixed(2)}ms, Memory: ${memoryUsed.toFixed(2)}MB`);
   ...
   * 統計情報を計算
   */
  private calculateStats(values: number[]): { min: number; max: number; avg: number; p50: number; p95: number; p99: number } {
    if (values.length === 0) {
  ...
  private generateReport(): void {
    ...
    const meetsMemoryLimit = this.results.memoryUsage.peak <= TEST_CONFIG.REQUIREMENTS.MEMORY_LIMIT_MB;
    console.log(
    ...
    console.log(`   Success rate: ${(((this.results.summary.totalTests - this.results.summary.failedTests) / this.results.summary.totalTests) * 100).toFixed(2)}%`);

    const allRequirementsMet = meetsResponseTime && meetsMemoryLimit && this.results.summary.failedTests === 0;
