...
const BASE_URL = process.env.BASE_URL || (VPC_MODE ? 'http://localhost:3000' : 'http://localhost:3000');
...
async function testImageVectorSearch(embedding: number[]) {
  ...
  try {
    ...
    assertEqual(response1.status, 200, 'HTTP status should be 200');
    assert(data1.success === true || data1.data !== undefined, 'Response should contain data or success=true');
