...
export const isValidVectorDimensions = (dimensions: number): boolean => {
  return dimensions === MIN_VECTOR_DIMENSIONS;
};
 ...
 */
export const meetsConfidenceThreshold = (score: number): boolean => {
...
export const filterByConfidence = (results: SearchResult[]): SearchResult[] => {
  return results.filter((result) => meetsConfidenceThreshold(result.score));
};
