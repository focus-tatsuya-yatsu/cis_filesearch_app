  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // 圧縮統計を表示
        const savingsMB =
          (compressionResult.originalSize - compressionResult.compressedSize) / 1024 / 1024
        if (savingsMB > 0.1) {
          console.log(
            `[Image Compression] Saved ${savingsMB.toFixed(2)}MB (${compressionResult.compressionRatio.toFixed(1)}% reduction)`
          )
        }
          ...
          {!selectedFile && !isUploading && (
            <>
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-400 mt-2">JPEG, PNG (最大5MB)</p>
              </div>
            </>
          )}
