...
export class BlueGreenMigration {
  ...
  constructor(client: Client, blueIndexName: string, greenIndexName: string, alias: string) {
    this.client = client
  ...
  private async validateGreenIndex(): Promise<void> {
    ...
    for (const rule of this.validationRules) {
      const result = await rule.validate(this.client, this.blueIndex.name, this.greenIndex.name)
...
interface ValidationRule {
  validate(client: Client, blueIndex: string, greenIndex: string): Promise<ValidationResult>
}
...
class DocumentCountValidation implements ValidationRule {
  async validate(client: Client, blueIndex: string, greenIndex: string): Promise<ValidationResult> {
    ...
    return {
      ...
      message:
        diff <= threshold ? 'Document counts match' : `Document count mismatch: ${diff} documents`,
      details: {
        blue: blueTotal,
        green: greenTotal,
        diff,
...
class SampleDataValidation implements ValidationRule {
  async validate(client: Client, blueIndex: string, greenIndex: string): Promise<ValidationResult> {
    ...
    const foundCount = greenCheck.body.docs.filter((doc: any) => doc.found).length
...
class SchemaValidation implements ValidationRule {
  async validate(client: Client, blueIndex: string, greenIndex: string): Promise<ValidationResult> {
    ...
    return {
      passed: isValid,
      message: isValid ? 'Schema validation passed' : 'k-NN field not properly configured',
      details: {
        imageEmbeddingField,
      },
    }
