...
export const runConcurrentBenchmark = async (
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    console.log(
      `[Concurrent Benchmark] Batch ${batch + 1}/${batches} complete`
    );
  }
...
export const printBenchmarkReport = (result: BenchmarkResult): void => {
  ...
  console.log('\n## Throughput');
  console.log(
    `  Queries/Second: ${result.metrics.throughput.queriesPerSecond.toFixed(2)}`
  );
  console.log(
    `  Avg Query Time: ${result.metrics.throughput.avgQueryTime.toFixed(2)}ms`
  );

  console.log('\n## Cache Performance');
  console.log(
    `  Hit Rate:    ${(result.metrics.cache.hitRate * 100).toFixed(2)}%`
  );
  ...
  console.log('\n## Reliability');
  console.log(
    `  Success Rate: ${(result.metrics.reliability.successRate * 100).toFixed(2)}%`
  );
  console.log(`  Error Count:  ${result.metrics.reliability.errorCount}`);
  console.log(
    `  Error Rate:   ${(result.metrics.reliability.errorRate * 100).toFixed(2)}%`
  );
...
export const compareBenchmarks = (
  ...
  const latencyImprovement =
    ((baseline.metrics.latency.p95 - current.metrics.latency.p95) /
      baseline.metrics.latency.p95) *
    100;

  const throughputImprovement =
    ((current.metrics.throughput.queriesPerSecond -
