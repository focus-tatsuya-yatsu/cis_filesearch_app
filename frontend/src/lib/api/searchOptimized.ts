 ...
 * 検索実行（キャッシュ対応）
 */
export const searchWithCache = async (params: SearchParams): Promise<SearchResponse> => {
  // キャッシュキーを生成
 ...
 * 複数ページを一括プリフェッチ
 */
export const prefetchPages = async (params: SearchParams, pageCount: number = 3): Promise<void> => {
 ...
 * 複数の検索クエリを効率的に実行
 */
export const batchSearch = async (queries: SearchParams[]): Promise<SearchResponse[]> => {
  console.log(`[Batch Search] Processing ${queries.length} queries`)

  const results = await Promise.allSettled(queries.map((params) => searchWithCache(params)))
