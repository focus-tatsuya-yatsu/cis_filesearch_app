...
const getErrorMessage = (statusCode: number, errorData?: SearchError): string => {
  // カスタムエラーコードのチェック
  if (
    errorData?.code === 'PAGINATION_LIMIT_EXCEEDED' ||
    errorData?.error === 'PAGINATION_LIMIT_EXCEEDED'
  ) {
  ...
  switch (statusCode) {
    ...
    case 500:
      ...
      if (
        errorData?.message?.includes('max_result_window') ||
        errorData?.error?.includes('max_result_window')
      ) {
...
const isRetryableError = (statusCode: number): boolean => [429, 502, 503, 504].includes(statusCode)
...
): response is ApiErrorResponse => 'statusCode' in response && 'userMessage' in response
