  ...
  async performHealthCheck(indexName: string): Promise<HealthCheckResult> {
    ...
    try {
      ...
      this.recordMetric(
        ...
        queryLatency,
        'Milliseconds',
        { IndexName: indexName, QueryType: 'HealthCheck' }
      );
        ...
        if (task.completed) {
          ...
          if (task.error) {
            await this.sendAlert({
              ...
              message: `Reindex task ${taskId} failed: ${JSON.stringify(
                task.error
              )}`,
            });
          }
  ...
  async getDashboardMetrics(
    ...
    try {
      ...
      const indexingSuccessRate =
        indexingTotal > 0
          ? ((indexingTotal - indexingFailed) / indexingTotal) * 100
          : 100;
  ...
  async compareIndices(
    ...
    const comparison = {
      ...
      diff: {
        ...
        searchSuccessRate:
          greenMetrics.searchSuccessRate - blueMetrics.searchSuccessRate,
      },
    };
...
export class SLATracker {
  ...
  getSLAStatus(sla: string): {
    ...
    const met = sla.includes('latency')
      ? current <= target
      : current >= target;
