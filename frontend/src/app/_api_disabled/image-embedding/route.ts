...
async function exponentialBackoff(attemptNumber: number): Promise<void> {
  ...
  console.log(
    `[Bedrock] Retrying after ${totalDelay.toFixed(0)}ms (attempt ${attemptNumber + 1}/${MAX_RETRIES})`
  )
...
async function generateImageEmbedding(imageBase64: string): Promise<number[]> {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(
        `[Bedrock] Successfully generated embedding (${responseBody.embedding.length} dimensions)`
      )
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (embedding) {
      ...
      console.log(
        `[Image Embedding API] Embedding generated successfully in ${duration}ms, dimensions:`,
        embedding.length
      )
