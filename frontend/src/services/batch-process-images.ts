 ...
 */
async function saveEmbeddingToOpenSearch(s3Key: string, imageEmbedding: number[]): Promise<void> {
 ...
 */
async function processImage(s3Key: string, retryCount = 0): Promise<ProcessingResult> {
  try {
    ...
    console.log(
      `[Batch Process] Generated embedding: ${s3Key} (${imageEmbedding.length} dimensions)`
    )
    ...
    if (retryCount < CONFIG.RETRY_ATTEMPTS) {
      console.log(
        `[Batch Process] Retrying ${s3Key} (attempt ${retryCount + 1}/${CONFIG.RETRY_ATTEMPTS})`
      )
...
export async function batchProcessImages(): Promise<BatchProcessingStats> {
  ...
  try {
    ...
    const imageKeys = listResponse.Contents.filter(
      (obj) => obj.Key && /\.(jpg|jpeg|png)$/i.test(obj.Key)
    )
      .map((obj) => obj.Key!)
    ...
    for (let i = 0; i < imageKeys.length; i += CONFIG.BATCH_SIZE) {
      ...
      console.log(
        `[Batch Process] Processing batch ${Math.floor(i / CONFIG.BATCH_SIZE) + 1}/${Math.ceil(imageKeys.length / CONFIG.BATCH_SIZE)}`
      )

      const batchResults = await Promise.all(batch.map((key) => processImage(key)))
      ...
      console.log(
        `[Batch Process] Progress: ${stats.processed}/${stats.total} (${stats.successful} successful, ${stats.failed} failed)`
      )
    }
